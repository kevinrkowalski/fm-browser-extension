<script>
	...
	const activeFilterClasses = 'bg-red-700 text-white';
	const nonActiveFilterClasses = 'bg-white hover:bg-neutral-0';
	...
	const handleColorSchemeSwitch = () => {
		...
		isDarkMode = newMode;
		const colorScheme = newMode ? 'dark' : 'light';
	...
	onMount(() => {
		...
		const isDark = storedColorScheme === 'dark';
		isDarkMode = isDark;
		window.document.documentElement.classList.toggle('dark', isDark);
		window.localStorage.setItem('colorScheme', storedColorScheme);
	});
</script>

<main class="container mx-auto px-4 xl:px-48">
	<div
		class="mt-8 mb-20 flex justify-between rounded-2xl bg-white p-3 shadow-sm dark:bg-neutral-800"
	>
		<svg xmlns="http://www.w3.org/2000/svg" width="179" height="41" fill="none" viewBox="0 0 179 41"
			...
			><path
				fill="currentColor"
				class="text-[#091540] dark:text-white"
		...
		<button
			type="button"
			class="cursor-pointer rounded-xl bg-neutral-100 p-3 hover:bg-neutral-300 focus:outline-red-700 dark:bg-neutral-700"
			onclick={handleColorSchemeSwitch}
		>
			{#if isDarkMode}
	...
	<div class="">
		<div class="flex flex-wrap justify-center gap-4 md:justify-between">
			<h2 class="text-center text-3xl font-bold">Extension List</h2>
			<div class="flex items-center gap-3">
				<button
					type="button"
					class={`cursor-pointer rounded-full px-4 py-2 shadow-sm focus:outline-red-700 ${activeFilter === 'all' ? activeFilterClasses : nonActiveFilterClasses}`}
					data-filter="all"
				...
				<button
					type="button"
					class={`cursor-pointer rounded-full px-4 py-2 shadow-sm focus:outline-red-700 ${activeFilter === 'active' ? activeFilterClasses : nonActiveFilterClasses}`}
					data-filter="active"
					onclick={handleChangeFilter}
				>
					Active
				...
				<button
					type="button"
					class={`cursor-pointer rounded-full px-4 py-2 shadow-sm focus:outline-red-700 ${activeFilter === 'inactive' ? activeFilterClasses : nonActiveFilterClasses}`}
		...
		<Extensions filter={activeFilter} />
	</div>
</main>
